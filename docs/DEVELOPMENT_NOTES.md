...
- `Queue URL Input` 根对象带有 `Mask`，不能照搬普通 Cycle Button 的透明点击底做法，把作为遮罩源的 `Image.color.a` 设为 0；全透明遮罩源可能无法写入有效模板，结果会把子级链条图标、“视频链接”文字和输入文字全部裁掉。需要隐藏第三格自己的矩形时，必须保持遮罩源为不透明颜色（当前使用 `Color.white`），再用 `Mask.showMaskGraphic = false` 隐藏它；可见背景仍由外层共用 `BG/Fill` 绘制。第三格图标必须复用 `CreateOrFindControlIcon`，避免与前两格的尺寸、位置、材质或启用规则再次分叉。
...
- GitHub 当前正式基线是 **YamaPlayer PC / 桌面端 v1.10**，由 `beta13.43` 晋升；Release tag 为 `v1.10`，正式包为 `PaulKoiPlayer-YamaBiliDanmakuV3-1.10.zip`。该包使用已经验收的金黑色 UI，不包含 iwaSync3、VizVid 或 Tablet 线。
- 本地 `E:\paulkoiplayer\1.10.zip` 与正式 Release 资产内容一致，SHA-256 为 `D905E3BC0C657F4D926BBD427FB82E0E4B8EE9E728D36A517DC9A951CE8CFBE6`。
...
- 集成的“视频链接”输入框由生成器自动绑定到 Queue 输入和 Pages 组件；YamaPlayer 原有 Top/Bottom URL Input 仍是独立字段，需要世界作者按实际布局手动拖入，不能用组件顺序或名字猜测。
...
本轮结束时工作树不是干净的。`Editor/YamaBiliDanmakuRigBuilder3.cs` 保存本地黑白 Alpha；`YamaBiliDanmakuTabletV3/Editor/YamaBiliDanmakuTabletRigBuilder3.cs` 与 `YamaBiliDanmakuTabletV3/Runtime/YamaBiliDanmakuTabletModule3.cs` 也有未发布修改；`Textures/` 和测试截图是未跟踪文件。新对话必须先执行 `git status --short` 和逐文件 diff，不能 `reset --hard`、不能 checkout 覆盖，也不能把这些文件顺手混入其他提交。

接手后的第一步应依次是：确认用户要处理的是正式 v1.10、黑白 Alpha、Tablet、iwaSync3 还是 VizVid；读取本文件第 15 至 17 节；检查实际使用的 YamaPlayer 源码版本；需要排查播放状态时先读取最新 VRChat Player log，再对照请求 URL、manifest、Queue、当前 vcrid、Owner、Stop/End 和 pending 标志。没有本机 Unity 环境时只能做源码、包结构和静态检查，不能把“能编译/能在 VRChat 运行”写成已经验证。
