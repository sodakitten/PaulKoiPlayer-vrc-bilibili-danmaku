...
## 21. 正式 1.11 前端安全修复测试线（frontend-fix-alpha1.0）

> 2026-09-24 发布补记：用户已明确授权将这两个现有 ZIP 作为测试附件上传到 v1.11 Release，并更新主分支源码。它们没有晋升为正式稳定包；原 v1.11 标签和四个已有附件不变。以下“本地制作”记录保留作为来源追溯，包内 README 的未发布文字描述的是打包时状态。

本线只处理本次核查确定的前端异步请求和编辑器配置问题，不改后端。普通版从正式 `1.11.zip` 提取；黑金版用该 ZIP 的共同资源，加上正式 Release 附件 `黑金版.unitypackage` 中的黑金 Editor / Pages 源码。YamaPlayer 继续固定参考本机 `net.kwxxw.yama-stream-1.5.18.zip`。本线不包含字幕 alpha、beta14.03 同步恢复实验、Tablet、iwaSync3 或 VizVid 未发布内容。

为避免覆盖仓库里尚未提交的字幕 Runtime 和其他测试工作，本线源码、正式基线、测试脚本均隔离在 `E:\paulkoiplayer\frontend-fix-alpha1.0-work`。本地制作阶段只追加本节文档，未覆盖字幕工作树。获发布授权后，从远端 main 单独检出提交本线三个黑白源码文件、文档、测试脚本及黑金视觉差异补丁；原工作树及其未发布改动仍不覆盖、不混入提交。

### 修复边界与不可重犯的规则

- `YamaBiliPagesPlaylist3`：下载结果不能只凭 URL 相同就应用。当前播放展开必须在应用时仍属于相同播放 URL、相同 Controller Owner 且本地仍持有该 Owner；队列标题解析必须确认对应队列条目还在。完成、取消、业务超时都必须让请求身份失效，避免旧 manifest 重新展开、超时后重复入队或旧歌名覆盖新输入。
- 主动输入的 Enqueue 请求和“展开当前播放”不是同一种生命周期。正常换片不应取消用户主动加入队列；同步 manifest 的选中 vcrid 变化也不应取消或重复下载同一份清单。事件、轮询和成功/失败回调均检查相关上下文。
- `YamaBiliDanmakuModule3`：成功与失败回调都验证请求代次、当前播放 URL 和音频模式；失效错误不能触发当前内容的 fallback。YamaPlayer 1.5.18 的 `Track` 先更新、同步 `_url` 后序列化，因此识别当前播放时优先使用 `Track.GetVRCUrl()`。`Stopped && IsLoading` 是正常加载过渡，不能一概当作已停止清空请求。
- 两个组件分别保持最多一个 SDK 字符串下载在途。取消逻辑请求时不能假装 SDK 下载已经取消；必须排空旧回调，再发最新待处理请求，防止 A → B → A 的同 URL 旧结果被误认。15 秒业务超时仅结束该业务请求，不能释放还未收到回调的 SDK 请求身份。代价是新请求可能等待旧下载结束；若 SDK 完全不回调或 Udon 已 Halt，本线不声称能自愈。
- manifest 和元数据分别从真正发出 SDK 请求时起计时。旧的延迟计时事件不能提前终止新一轮元数据请求。超时后的晚到结果必须丢弃，不能再回填或再次入队。
- `YamaBiliDanmakuRigBuilder3`：应用皮肤或重连引用不得覆盖既有 Helper / Pages 的 URL 前缀、后端地址、vcrid 目录配置、自动播放、轮询和跑马灯设置；默认配置只给新建组件。保留手动引用和正式版精确 1.5.18 URL Input 自动绑定规则，不引入宽泛猜测。

两种视觉分别保持原正式版本，不借机重绘 UI、改变选中样式或调渲染层级。两个 Shader、字体、URL Prefix Helper Runtime、构建期 vcrid 生成器逐文件哈希不变；弹幕解析、mode=4 歌词渲染、轨道、文字池、暂停时间轴和镜子逻辑未重写。普通队列、多 P 和网易云歌单播放策略不改，仅在失效异步请求入口加保护。此版不是远端“完全没有反应 / 严重运行时异常”的已验证修复。

### 本地交付与验证

- 普通版：`E:\paulkoiplayer\1.11-frontend-fix-alpha1.0.zip`，`3052026` 字节，SHA-256 `39A9A2F4E2606DC8D337B17C54393430F296E43D9345969C2C8BC77591F42D4E`。
- 黑金版：`E:\paulkoiplayer\1.11-frontend-fix-alpha1.0-black-gold.zip`，`3052059` 字节，SHA-256 `0CA01ACEEDCBDFC54DE6076F7E6AB2E03CD6529068F6708ADB6E35EC9401AFF8`。
- 每包保留正式 9 文件目录，只有上述 3 个 C# 文件变化，另附根目录安装/回退说明，共 10 个文件；压缩包已逐条目读回并校验与源文件哈希一致。
- 普通版通过 730 项源码/不变项检查，黑金版通过 728 项；两版之间的视觉代码差异与正式基线一致。两版各通过 43 项基于实际请求方法的 C# 行为断言，覆盖旧回调、A → B → A、超时、换片、所有权、队列删除、元数据及 fallback。测试脚本为工作目录中的 `Verify-FrontendFix.ps1`，参数 `-Variant normal` 或 `-Variant gold`。
- Roslyn 全文件语法检查与抽取方法测试不等于完整 Unity/C# 工程编译或 UdonSharp 编译；测试中的 Unity/SDK I/O、解析和 UI 为桩。本机无 Unity，本线仍待 Unity/UdonSharp、双客户端、VR/PC 与镜子实测，不能标为正式验收通过。

导入前备份工程；在原目录仅替换这三个同名 `.cs`，保留 `.meta`、GUID、组件引用和 UdonSharp Program Asset，不删除重建组件，不并排导入两个皮肤的同名 Runtime。全量重新编译 UdonSharp 并确认 Console 无错误后才能 Build & Test；不必重新生成 UI。回退也必须还原相应源码后重新编译，不能上传旧编译产物。至少实测解析中清空/换片/换 Owner、慢网络超时、重复同链接、正常排队、网易云单曲/整张歌单、B 站多 P 后继续普通视频、Stop/End、晚加入及皮肤应用后自定义设置保留。后端沿用正式 1.11，无需部署变更。

### 公开源码与测试包的对应关系

主分支 `Runtime/`、`Editor/` 中本次三个文件对应普通黑白测试包，不再代表未经修改的正式 1.11。稳定版请取 v1.11 Release 原正式附件，不要把 main 分支下载误当作稳定包。黑金测试包与普通包共享本次修复，原有黑金视觉差异记录在 `docs/variants/black-gold-ui.patch`；需要还原黑金源码时，在独立副本根目录应用此补丁，不要同时导入两份同名 Runtime。具体安装、校验和测试步骤见 `docs/FRONTEND_FIX_ALPHA1.0.md`，可复核脚本为 `tests/Verify-FrontendFix.ps1`。
