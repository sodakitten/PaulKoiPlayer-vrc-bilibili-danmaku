...
namespace IwaBiliDanmakuV3.Editor
{
  public static class IwaBiliDanmakuRigBuilder3
  {
    ...
    private const float CanvasLocalY = 1.001f;
    private const float CanvasLocalZ = -0.02f;
    private const string DefaultUrlPrefix = "https://danmaku.paulkoishi.com/player/?url=";
    private const string DefaultPrefabPath = "Assets/IwaBiliDanmakuV3/Prefabs/Bili Danmaku Module.prefab";
    private const string OutlineMaterialPath = "Assets/IwaBiliDanmakuV3/Materials/Bili Danmaku TMP Outline.mat";
    private const string ButtonMaterialPath = "Assets/IwaBiliDanmakuV3/Materials/Bili Danmaku UI Button.mat";
    private const string MirrorReadableShaderName = "IwaBiliDanmaku/TMP Mirror Readable";
    private const string ButtonShaderName = "IwaBiliDanmaku/UI Button";
    ...
    [MenuItem("PaulKoiPlayer/iwaSync3/Create Bili Danmaku Module", false, 2000)]
    public static void CreateRig()
    {
      VideoCore core = FindTargetVideoCore();
      if (core == null)
      {
        EditorUtility.DisplayDialog("iwa Bili Danmaku", "Select a iwaSync3 object, or an object under iwaSync3 that contains a core.", "OK");
        return;
      }
    ...
    [MenuItem("GameObject/iwaSync3/Bili Danmaku Module", false, 41)]
    public static void CreateRigFromGameObjectMenu()
    {
      CreateRig();
    }

    [MenuItem("GameObject/iwaSync3/Bili Danmaku Module", true)]
    public static bool ValidateCreateRigFromGameObjectMenu()
    {
      return FindTargetVideoCore() != null;
    }

    [MenuItem("PaulKoiPlayer/iwaSync3/Save Selected Bili Danmaku Module As Prefab", false, 2001)]
    public static void SaveSelectedAsPrefab()
    {
      GameObject selected = Selection.activeGameObject;
      if (selected == null || !HasDanmakuModule(selected))
      {
        EditorUtility.DisplayDialog("iwa Bili Danmaku", "Select a Bili Danmaku Module object first.", "OK");
        return;
      }
      ...
      string path = EditorUtility.SaveFilePanelInProject(
        ...
        "Assets/IwaBiliDanmakuV3/Prefabs");
      ...
      if (prefab == null)
      {
        EditorUtility.DisplayDialog("iwa Bili Danmaku", "Failed to save prefab.", "OK");
        return;
      }
    ...
    [MenuItem("PaulKoiPlayer/iwaSync3/Create Package Prefab Asset", false, 2002)]
    public static void CreatePrefabAsset()
    {
      ...
      try
      {
        ...
        if (prefab == null)
        {
          EditorUtility.DisplayDialog("iwa Bili Danmaku", "Failed to create prefab asset.", "OK");
          return;
        }
    ...
    private static GameObject BuildRig(VideoCore core, Transform parent)
    {
      GameObject root = new GameObject("Bili Danmaku Module (iwaSync3)", typeof(RectTransform));
      Undo.RegisterCreatedObjectUndo(root, "Create Bili Danmaku Module");
      ...
      System.Type moduleType = FindType("IwaBiliDanmakuV3.IwaBiliDanmakuModule3");
      if (moduleType == null)
      {
        EditorUtility.DisplayDialog(
          "iwa Bili Danmaku",
          "IwaBiliDanmakuModule3 type was not found. Make sure the Runtime script has compiled successfully, then reimport this package or restart Unity.",
          "OK");
      ...
      if (module == null)
      {
        EditorUtility.DisplayDialog(
          "iwa Bili Danmaku",
          "Failed to add UdonSharp component. Try creating a U# Program Asset manually from Assets/IwaBiliDanmakuV3/Runtime/IwaBiliDanmakuModule3.cs, then run this menu again.",
          "OK");
        UnityEngine.Object.DestroyImmediate(root);
        return null;
      }

      Component urlPrefixHelper = CreateOrFindUrlPrefixHelper(root, core, urlPrefixToggleButtonLabel);
    ...
    private static Component CreateOrFindUrlPrefixHelper(GameObject root, VideoCore core, TextMeshProUGUI urlPrefixToggleButtonLabel)
    {
      System.Type helperType = FindType("IwaBiliDanmakuV3.IwaBiliUrlPrefixHelper3");
      if (helperType == null)
      {
        Debug.LogWarning("iwa Bili Danmaku: IwaBiliUrlPrefixHelper3 type was not found. URL prefix helper was not created.");
        return null;
      }
      ...
      if (helper == null)
      {
        GameObject helperObject = new GameObject("Bili URL Prefix Helper (iwaSync3)");
        helperObject.transform.SetParent(root.transform, false);

        helper = AddUdonSharpComponentForType(helperObject, helperType);
        if (helper == null)
        {
          Debug.LogWarning("iwa Bili Danmaku: failed to add Bili URL Prefix Helper.");
          UnityEngine.Object.DestroyImmediate(helperObject);
          return null;
        }
      }

      WireUrlPrefixHelper(helper, core, urlPrefixToggleButtonLabel);
      return helper;
    }
    ...
    private static void EnsureVrcUiShape(GameObject canvasObject)
    {
      ...
      if (uiShapeType == null)
      {
        Debug.LogWarning("iwa Bili Danmaku: VRC_UiShape type was not found. Add VRC_UiShape to Danmaku Controls Canvas manually if the buttons are not clickable in VRChat.");
        return;
      }
    ...
    private static VideoCore FindTargetVideoCore()
    {
      GameObject selected = Selection.activeGameObject;
      if (selected != null)
      {
    ...
    private static Transform FindIwaRoot(Transform coreTransform)
    {
      ...
      Transform current = coreTransform;
      while (current.parent != null)
      {
        current = current.parent;
        if (current.GetComponentInChildren<VideoCore>(true) != null)
        {
          return current;
        }
      }

      return coreTransform.parent;
    }
    ...
    private static Component AddUdonSharpComponentForType(GameObject gameObject, System.Type moduleType)
    {
      ...
      {
        Debug.LogError("iwa Bili Danmaku: AddComponent failed: " + exception);
        return null;
      }
    }
        ...
        for (int t = 0; t < types.Length; t++)
        {
          ...
          for (int m = 0; m < methods.Length; m++)
          {
            ...
            try
            {
              ...
              if (component != null)
              {
                Debug.Log("iwa Bili Danmaku: Added UdonSharp component via " + type.FullName + "." + method.Name);
                return component;
              }
            }
            catch (System.Exception exception)
            {
              Debug.LogWarning("iwa Bili Danmaku: UdonSharp add helper failed: " + exception.Message);
            }
          }
        }
      }
    ...
    [MenuItem("PaulKoiPlayer/iwaSync3/Wire Selected Bili Danmaku Module", false, 2003)]
    public static void WireSelectedModule()
    {
      GameObject selected = Selection.activeGameObject;
      if (selected == null)
      {
        EditorUtility.DisplayDialog("iwa Bili Danmaku", "Select the Bili Danmaku Module object first.", "OK");
        return;
      }

      Component module = selected.GetComponent("IwaBiliDanmakuModule3");
      if (module == null)
      {
        EditorUtility.DisplayDialog("iwa Bili Danmaku", "Selected object does not have IwaBiliDanmakuModule3. Add it in Inspector first.", "OK");
        return;
      }
      ...
      WireModule(module, core, laneRoot, status, displayAreaButtonLabel, danmakuToggleButtonLabel, displayAreaButton, danmakuToggleButton, pool);
      Component urlPrefixHelper = CreateOrFindUrlPrefixHelper(selected, core, urlPrefixToggleButtonLabel);
      WireModuleUrlPrefixControls(module, urlPrefixHelper, urlPrefixToggleButtonLabel, urlPrefixToggleButton);
      ApplyVisualStyleFromModule(module, pool);
      EditorUtility.DisplayDialog("iwa Bili Danmaku", "References wired.", "OK");
    }

    [MenuItem("PaulKoiPlayer/iwaSync3/Apply Selected Bili Danmaku Visual Style", false, 2005)]
    public static void ApplySelectedVisualStyle()
    {
      ...
      if (module == null)
      {
        EditorUtility.DisplayDialog("iwa Bili Danmaku", "Selected object does not have IwaBiliDanmakuModule3.", "OK");
        return;
      }

      ApplyVisualStyleFromModule(module, selected.GetComponentsInChildren<TextMeshProUGUI>(true));
      EditorUtility.DisplayDialog("iwa Bili Danmaku", "Visual style applied.", "OK");
    }

    [MenuItem("PaulKoiPlayer/iwaSync3/Wire Selected Bili URL Prefix Helper", false, 2004)]
    public static void WireSelectedUrlPrefixHelper()
    {
      GameObject selected = Selection.activeGameObject;
      if (selected == null)
      {
        EditorUtility.DisplayDialog("iwa Bili Danmaku", "Select the Bili URL Prefix Helper object first.", "OK");
        return;
      }

      Component helper = selected.GetComponent("IwaBiliUrlPrefixHelper3");
      if (helper == null)
      {
        EditorUtility.DisplayDialog("iwa Bili Danmaku", "Selected object does not have IwaBiliUrlPrefixHelper3.", "OK");
        return;
      }
      ...
      WireUrlPrefixHelper(helper, FindTargetVideoCore(), urlPrefixToggleButtonLabel);
      EditorUtility.DisplayDialog("iwa Bili Danmaku", "URL prefix helper references wired.", "OK");
    }

    private static void WireUrlPrefixHelper(Component helper, VideoCore core, TextMeshProUGUI urlPrefixToggleButtonLabel)
    {
      if (helper == null) return;

      VRCUrlInputField addressInput = FindIwaSyncAddressInput(core);
      ...
      if (addressProperty != null && addressInput != null) addressProperty.objectReferenceValue = addressInput;
    ...
    private static void AddPrefixInputEventTriggers(VRCUrlInputField inputField, Component helper, string eventName)
    {
      ...
      if (behaviour == null)
      {
        Debug.LogWarning("iwa Bili Danmaku: URL prefix helper is not an UdonSharpBehaviour, cannot wire input click events.");
        return;
      }
    ...
    private static VRCUrlInputField FindIwaSyncAddressInput(VideoCore core)
    {
      if (core == null) return null;

      Transform root = FindIwaRoot(core.transform);
    ...
    private static void WireModule(Component module, VideoCore core, RectTransform laneRoot, TextMeshProUGUI status, TextMeshProUGUI displayAreaButtonLabel, TextMeshProUGUI danmakuToggleButtonLabel, Button displayAreaButton, Button danmakuToggleButton, TextMeshProUGUI[] rawPool)
    {
      ...
      SetBool(serialized, "_loadFromCurrentIwaSyncUrl", true);
    ...
    private static void AddModuleButtonClick(Button button, Component module, string eventName)
    {
      ...
      {
        Debug.LogWarning("iwa Bili Danmaku: module is not an UdonSharpBehaviour, cannot wire controls button.");
        return;
      }
    ...
    private static bool TryGetSendCustomEventAction(Component component, out UnityAction<string> sendEvent)
    {
      ...
      {
        Debug.LogWarning("iwa Bili Danmaku: failed to resolve backing UdonBehaviour, falling back to UdonSharp proxy. " + exception.Message);
      }
    ...
    private static Material GetOrCreateOutlineMaterial(TextMeshProUGUI[] rawPool, float outlineWidth, Color outlineColor)
    {
      ...
      if (material == null)
      {
        ...
        if (source == null)
        {
          Debug.LogWarning("iwa Bili Danmaku: could not find a TMP source material for outline. Component outline values were still applied.");
          return null;
        }
    ...
    private static Material GetOrCreateButtonMaterial()
    {
      ...
      if (shader == null)
      {
        Debug.LogWarning("iwa Bili Danmaku: UI button shader was not found. Button images may trigger the VRChat SDK built-in UI shader alert.");
        return material;
      }
    ...
    private static void ApplyTmpOutlineMaterial(Material material, float outlineWidth, Color outlineColor)
    {
      ...
      if (material.HasProperty("_IWBDMForceMirrorFlip")) material.SetFloat("_IWBDMForceMirrorFlip", 0f);
    }
    ...
    private static void ApplyMirrorReadableShader(Material material)
    {
      ...
      if (shader == null)
      {
        Debug.LogWarning("iwa Bili Danmaku: mirror-readable TMP shader was not found. Keeping the current TMP shader.");
        return;
      }
    ...
    private static void SetVRCUrl(SerializedProperty property, string url)
    {
      ...
      {
        Debug.LogWarning("iwa Bili Danmaku: could not set VRCUrl serialized value for " + property.propertyPath + ". Set Url Prefix manually in Inspector.");
      }
    }
    ...
    private static bool HasDanmakuModule(GameObject gameObject)
    {
      ...
      return gameObject.GetComponent("IwaBiliDanmakuModule3") != null;
    }

    [MenuItem("PaulKoiPlayer/iwaSync3/Fix Bili Danmaku U# Program Asset", false, 2006)]
    public static void FixProgramAssetMenu()
    {
      System.Type moduleType = FindType("IwaBiliDanmakuV3.IwaBiliDanmakuModule3");
      if (moduleType == null)
      {
        EditorUtility.DisplayDialog("iwa Bili Danmaku", "Runtime type not found. Run Assets > Reimport All first and check Console for C# compile errors.", "OK");
        return;
      }
      ...
      {
        EditorUtility.DisplayDialog("iwa Bili Danmaku", "Failed to create U# Program Asset.", "OK");
        return;
      }

      Selection.activeObject = asset;
      EditorGUIUtility.PingObject(asset);
      EditorUtility.DisplayDialog("iwa Bili Danmaku", "U# Program Assets are ready. Now run UdonSharp > Compile All UdonSharp Programs, then create the module again.", "OK");
    }
    ...
    private static UnityEngine.Object EnsureUdonSharpProgramAsset(System.Type moduleType)
    {
      ...
      if (script == null)
      {
        Debug.LogError("iwa Bili Danmaku: cannot find " + moduleType.Name + ".cs MonoScript.");
        return null;
      }
      ...
      {
        Debug.LogError("iwa Bili Danmaku: cannot find UdonSharp.UdonSharpProgramAsset type.");
        return null;
      }

      string basePath = "Assets/IwaBiliDanmakuV3/Runtime/" + moduleType.Name + ".asset";
      ...
      if (asset == null)
      {
        Debug.LogError("iwa Bili Danmaku: failed to create UdonSharpProgramAsset instance.");
        return null;
      }
      ...
      if (source == null)
      {
        Debug.LogError("iwa Bili Danmaku: UdonSharpProgramAsset has no sourceCsScript property in this SDK version.");
      ...
      Debug.Log("iwa Bili Danmaku: Created U# Program Asset at " + path);
      return asset;
    }
    ...
    private static void TryInvokeAssetMethod(UnityEngine.Object asset, string methodName)
    {
      ...
      {
        Debug.LogWarning("iwa Bili Danmaku: " + methodName + " failed: " + exception.Message);
      }
    }
